...
--za uskladjivanje taktova koji kasne sa expected top
signal warmup_counter : unsigned(7 downto 0) := (others => '0');
signal warmup_done    : std_logic := '0';
constant PIPELINE_LATENCY : natural := 8;
...
begin
    ...
    begin
        if rising_edge(clk) then
            if start = '1' and warmup_done = '0' then
                if warmup_counter = PIPELINE_LATENCY - 1 then
                    warmup_done <= '1';
                else
    ...
    begin
        if(rising_edge(clk)) then
            if(start = '1' and warmup_done = '1') then
                address_output_bram <= std_logic_vector(unsigned(address_output_bram) + to_unsigned(1,ADDR_SIZE));
            end if;
        end if;         
    end process; 
